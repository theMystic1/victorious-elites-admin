        ...
        {dontShow ? null : (
          <div className="flex items-center gap-0 h-12  border border-gray-100 rounded-lg ">
            {schoolPaths?.map((path, i) => (
              <button
                key={path.value}
                className={`${pathname === path.value ? "bg-black text-white" : ""} h-full w-full  p-1.5 ${i === 0 ? "rounded-l-lg" : i === schoolPaths.length - 1 ? "rounded-r-lg" : "border-l border-r border-gray-200"} shadow-lg cursor-pointer`}
